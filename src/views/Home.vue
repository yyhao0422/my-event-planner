  ...
  methods: {
    ...
    async getRoomNames() {
      this.events.forEach(async (event) => {
        const room = await API.graphql(
          graphqlOperation(getRoom, { id: event.room_id })
        );
    ...
    async showMyTickets() {
      ...
      try {
        ...
        this.events = results.data.listEvents.items.filter((event) =>
          event.tickets.some(
            (ticket) => ticket.student_id === signedin_student_id
          )
        );
        // Sorting items by date Asc
        this.events.sort((a, b) => {
    ...
    async showMyPlannedEvents() {
      ...
      try {
        ...
        const myEvents = await API.graphql(
          graphqlOperation(listEvents, {
            filter: { event_owner: { eq: signedin_student_id } },
          })
        );
