      ...
      endTimeRules: [
        ...
        (value) => {
          ...
          if (start_hour >= end_hour)
            return "Event cannot start after it has finished";
  ...
  methods: {
    ...
    async findRooms() {
      ...
      if (this.event_datetime_start == "") {
        this.findRoomsError =
          "Please select a start time before searching rooms";
        this.findRoomsFailed = true;
        return;
      }
      if (this.event_datetime_end == "") {
        this.findRoomsError =
          "Please select an end time before searching rooms";
        this.findRoomsFailed = true;
        return;
      }
      ...
      console.log(
        `Searching for rooms available from ${this.start_datetime.toISOString()} to ${this.end_datetime.toISOString()} with a capacity of at least ${
          this.total_tickets
        }`
      );
      ...
      await API.graphql(graphqlOperation(findSuitableRooms, variables))
        .then((response) => {
          ...
          if (response.data.findSuitableRooms.includes(successHeader)) {
            var strData = response.data.findSuitableRooms.slice(
              successHeader.length,
              response.data.findSuitableRooms.length - 1
            );
    ...
    calculateEventStartEndTimes() {
      ...
      try {
        ...
        eventTimeStart.setHours(
          parseInt(this.event_datetime_start.split(":")[0]) + 12
        ); // Convert to 24 hour clock
        eventTimeEnd.setHours(
          parseInt(this.event_datetime_end.split(":")[0]) + 12
        ); // Convert to 24 hour clock
